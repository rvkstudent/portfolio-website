...
(function() {
    ...
    async function handleSearch(query) {
        if (!query || query.length < 2) { dom.searchResults.innerHTML = ''; return; }
        const data = await searchCards(query);
        if (!data.results || data.results.length === 0) {
            dom.searchResults.innerHTML = '<div class="search-result-item">Ничего не найдено</div>';
            return;
        }
        ...
        dom.searchResults.querySelectorAll('.search-result-item').forEach(el => {
            el.addEventListener('click', () => {
                ...
                if (idx >= 0) { state.currentIndex = idx; showCard(idx); }
                else {
                    // Reload all cards to include the searched one
                    loadCards().then(() => {
                        state.currentIndex = 0;
                        showCard(0);
                    });
    ...
    function handleKeydown(e) {
        ...
        if (e.key === 'Escape') {
            if (!dom.searchPanel.classList.contains('hidden')) { toggleSearch(); return; }
        }
        if (!dom.searchPanel.classList.contains('hidden')) return;
        switch (e.key) {
            case ' ': e.preventDefault(); if (!state.isFlipped) handleShowAnswer(); break;
